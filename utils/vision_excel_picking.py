...
def _normalize_excel_value(val: Any) -> Any:
    """
    Intenta normalizar valores del Excel sin perder trazabilidad.
...
def _clean_record(rec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for k, v in rec.items():
        out[k] = _normalize_excel_value(v)
    return out
...
def build_excel_product_index(records: List[Dict[str, Any]]) -> Dict[str, List[Dict[str, Any]]]:
    """
    Indexa por artículo y conserva TODAS las filas del Excel para ese artículo.
    ...
    for rec in records:
        ...
        excel_item = {
            ...
            "cant_trabajada": _to_int(rec.get("cant_trabajada")),
            "diferencia": _to_int(rec.get("diferencia")),
            "kits": _normalize_excel_value(rec.get("kits")),
            # Registro completo del Excel, limpio, para trazabilidad total
            "excel_full": rec_clean,
        }

        idx.setdefault(articulo, []).append(excel_item)
...
def match_picking_with_excel(
    ...
    for prod in products:
        ...
        first_match = excel_matches[0]

        matched_products.append({
            "codigo_item_ocr": prod.get("codigo_item"),
            "codigo_item_excel": first_match.get("articulo"),
            "descripcion_ocr": prod.get("descripcion"),
            "descripcion_excel": first_match.get("descripcion"),
            "unidades_ocr": prod.get("unidades"),
            "cant_original_excel": first_match.get("cant_original"),
            "cant_trabajada_excel": first_match.get("cant_trabajada"),
            "diferencia_excel": first_match.get("diferencia"),
            "cliente_excel": first_match.get("cliente"),
            "direccion_excel": first_match.get("direccion"),
            "orden_compra_excel": first_match.get("orden_compra"),
            "shipping_excel": first_match.get("shipping"),
            "ruta_excel": first_match.get("ruta"),
            "estado_excel": first_match.get("estado"),
            "estado_orden_excel": first_match.get("estado_orden"),
            "descripcion_estado_excel": first_match.get("descripcion_estado"),
            "match_status": "matched",
            "excel_match_count": len(excel_matches),
            # compatibilidad simple: primer match
            "excel": first_match,
            # trazabilidad completa: todas las coincidencias del artículo
    ...
    return {
        ...
        "counts": {
            ...
            "excel_filtered_rows": len(filtered),
            "matched_products": sum(1 for p in matched_products if p["match_status"] == "matched"),
            "not_found_products": sum(1 for p in matched_products if p["match_status"] == "not_found_in_excel"),
        },
    }
