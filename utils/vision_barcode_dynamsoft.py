...
def _ensure_bgr(image: np.ndarray) -> np.ndarray:
    """
    Asegura salida en BGR para escritura/compatibilidad.
    """
    if image.ndim == 2:
...
def _preprocess_variants(image: np.ndarray) -> List[Tuple[str, np.ndarray]]:
    """
    Genera variantes suaves para mejorar la lectura de códigos 1D/2D.
    """
    ...
    variants.append(("gray", _ensure_bgr(gray)))

    # EqualizeHist sobre gray
    gray_eq = cv2.equalizeHist(gray)
    variants.append(("gray_eq", _ensure_bgr(gray_eq)))

    # Sharpen suave sobre original
    sharp = cv2.filter2D(image, -1, sharpen_kernel)
    variants.append(("sharp", sharp))

    # Equalize + sharpen
    gray_eq_sharp = cv2.filter2D(gray_eq, -1, sharpen_kernel)
    variants.append(("gray_eq_sharp", _ensure_bgr(gray_eq_sharp)))

    # CLAHE para levantar contraste local
    clahe = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(8, 8))
    gray_clahe = clahe.apply(gray)
    variants.append(("gray_clahe", _ensure_bgr(gray_clahe)))

    return variants


def _capture_single_path(
    image_path: Union[str, Path],
    variant_name: Optional[str] = None,
) -> Dict[str, Any]:
    """
    Ejecuta una lectura Dynamsoft sobre una ruta puntual.
    """
    img_path = Path(image_path)
...
def _capture_from_ndarray_variants(image: np.ndarray) -> Dict[str, Any]:
    """
    Ejecuta lectura Dynamsoft sobre varias variantes de la misma imagen
    ...
    for variant_name, variant_img in _preprocess_variants(image):
        ...
        try:
            ok = cv2.imwrite(tmp_path, variant_img)
            if not ok:
                variant_summaries.append(
                    {
                        ...
                        "status": "error",
                        "total": 0,
                        "error_message": "No se pudo escribir variante temporal",
                    }
                )
                continue

            partial = _capture_single_path(tmp_path, variant_name=variant_name)
    ...
    all_items = _dedupe_items_spatial(all_items)
...
def _capture_from_path(image_path: Union[str, Path]) -> Dict[str, Any]:
    """
    Ejecuta lectura consolidada a partir de una ruta de imagen.
    Usa variantes de preprocesamiento para robustecer lectura.
    """
    img_path = Path(image_path)
    ...
    if image is None:
        raise RuntimeError(f"No se pudo cargar la imagen: {img_path}")

    result = _capture_from_ndarray_variants(image)
...
def decode_barcode_dynamsoft(
    ...
    En lugar de evaluar una sola imagen temporal, prueba varias
    variantes suaves y consolida resultados.
    """
    if image is None:
        raise ValueError("La imagen recibida es None")
    ...
    # temp_suffix se conserva por compatibilidad hacia atrás
    _ = temp_suffix
...
def main() -> int:
    ...
    try:
        ...
        if args.json:
            ...
            print("\n=== RESULTADO DYNAMSOFT ===\n")
            print(f"Status: {result['status']}")
            print(f"Backend: {result.get('backend', 'hybrid')}")
            print(f"Imagen: {result.get('image_path')}")
            print(f"Total detectados: {result.get('total', 0)}")

            if result.get("variant_summaries"):
                print("\nResumen por variante:")
                for vs in result["variant_summaries"]:
                    print(
                        f"- {vs.get('variant')}: "
                        f"status={vs.get('status')} total={vs.get('total', 0)}"
                    )
            ...
            if result["status"] == "success":
                for idx, item in enumerate(result["items"], start=1):
                    ...
                    print(f"Texto:   {item.get('text')}")
                    if item.get("variant"):
                        print(f"Variante:{item.get('variant')}")
                    if item.get("bbox"):
                        print(f"BBox:    {item.get('bbox')}")
            elif result["status"] == "error":
                print(f"Error code: {result.get('error_code')}")
