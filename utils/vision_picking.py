...
class PickingSheetConfig:
    ...
    # Heurística espacial:
    # si Dynamsoft retorna bbox consistente, favorece códigos más a la derecha
...
def _run_shipping_barcode_readout(
    ...
    *,
    ...
    raw_result = decode_barcode_dynamsoft(
        img_bgr,
...
def read_picking_shipping(
    ...
    *,
    ...
    # Estrategia principal y única por ahora:
    # full image -> una sola llamada a Dynamsoft.
...
#     p = Path(image_path)
#     return str(p.with_name(f"{p.stem}_picking.json"))
...
#     p = Path(image_path)
#     return str(p.with_name(f"{p.stem}_picking_debug"))
...
def _build_parser():
    ...
    parser = argparse.ArgumentParser(
        description="Lee shipping/orden desde hoja de picking usando Dynamsoft."
    )
