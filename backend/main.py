...
def process_capture(payload: ProcessRequest):
    ...
    1) readout híbrido sobre el frame del evento
    2) picking_flow (summary + picking_shipping + closure_iterative)
    """
    try:
        event_dir = resolve_event_dir(payload.event_dir)

        frame_path = normalize_input_path(payload.picking_image)
        if frame_path is None:
            frame_path = resolve_target_frame(event_dir)

        if not frame_path.exists():
            raise HTTPException(status_code=404, detail=f"No existe picking_image/frame_path: {frame_path}")
        ...
        picking_shipping_json_path = PICKING_DIR / f"{frame_path.stem}_picking_shipping.json"
