...
def _filter_product_fillrate_matches_by_shipping(
    prod: Dict[str, Any],
    ...
    matches = out.get("fillrate_matches") or []
    ...
    for row in matches:
        ...
        row_shipping = row.get("shipping")
        if row_shipping is None:
            excel_full = row.get("excel_full") or {}
            row_shipping = excel_full.get("Shipping") or excel_full.get("shipping")

        if _shipping_variants(row_shipping) & target_variants:
            filtered_matches.append(row)

    if filtered_matches:
        out["fillrate_matches"] = filtered_matches
        out["cant_original_total"] = sum(_safe_int(r.get("cant_original"), 0) for r in filtered_matches)
        out["cant_trabajada_total"] = sum(_safe_int(r.get("cant_trabajada"), 0) for r in filtered_matches)
        out["diferencia_total"] = sum(_safe_int(r.get("diferencia"), 0) for r in filtered_matches)
        out["shipping_values"] = [
            _norm_shipping(r.get("shipping"))
            for r in filtered_matches
            if _norm_shipping(r.get("shipping"))
        ]
...
def filter_summary_by_shipping(
    ...
    filtered_products = [
        _filter_product_fillrate_matches_by_shipping(prod, target_shipping)
